# Rust Hex Editor - TODO List

## ✅ 実装済み

### 基本機能
- [x] gpuiライブラリの統合
- [x] 基本的なウィンドウ表示
- [x] 3カラムレイアウト（アドレス、Hex、ASCII）
- [x] 16進数表示とASCII表示
- [x] モノスペースフォントでの整列表示

### データモデル
- [x] バイト配列の管理 (`Vec<u8>`)
- [x] 16バイト/行のフォーマット
- [x] アドレス表示（8桁の16進数）
- [x] Hex値のフォーマット
- [x] ASCII文字の表示（制御文字は '.' で表示）
- [x] サンプルデータの生成

### ファイル操作
- [x] コマンドライン引数でファイルパスを受け取る
- [x] ファイルの読み込み (`load_file()`)
- [x] ファイル名のヘッダー表示
- [x] ファイルサイズの表示
- [x] エラーハンドリング（ファイル読み込み失敗時のフォールバック）

### UI/UX
- [x] ダークテーマ (#1e1e1e 背景)
- [x] スクロール可能なコンテンツ領域
- [x] インタラクティブなスクロールバー
  - [x] マウスホイール対応
  - [x] ドラッグ操作対応
  - [x] 常時表示モード
- [x] カラーコーディング
  - [x] アドレス: グレー (#808080)
  - [x] Hex値: グリーン (#00ff00)
  - [x] ASCII: ホワイト (#ffffff)

### カーソルとナビゲーション
- [x] カーソル位置の管理
  - [x] 現在のバイト位置を保持 (`cursor_position`)
  - [x] FocusHandleによるフォーカス管理
- [x] カーソルの視覚的表示
  - [x] 選択中のバイトをハイライト (青色 #4a9eff)
  - [x] HexペインとASCIIペインで同期表示
- [x] 基本的なキーボードナビゲーション
  - [x] 矢印キー（上下左右）でカーソル移動
  - [x] Focusableトレイト実装
  - [x] on_key_downイベント処理

### バイト編集機能
- [x] バイト編集モード
  - [x] Hexペインでの編集（0-9, A-F入力）
  - [x] ASCIIペインでの編集（印字可能文字）
  - [x] 入力値の検証
  - [x] Hexニブル管理（上位/下位ビット）
- [x] Hexペイン/ASCIIペインの切り替え（Tab）
- [x] 編集中の視覚的フィードバック
  - [x] 編集モード表示（ヘッダー）
  - [x] アクティブペインのハイライト（Hex: 青、ASCII: オレンジ）
  - [x] 非アクティブペインの暗い表示
- [x] 自動カーソル移動（入力後に次のバイトへ）

### ファイル保存機能
- [x] 変更追跡フラグ（has_unsaved_changes）
- [x] 上書き保存（Ctrl+S / Cmd+S）
- [x] 保存成功メッセージ表示
- [x] 未保存変更の視覚的表示
  - [x] タイトルに「*」表示
  - [x] バイト数に「(modified)」表示
- [x] 保存エラーハンドリング

---

## ❌ 未実装機能

### 🔴 高優先度（エディタとして必須）

#### 1. カーソルとナビゲーション（拡張） ✅ 完了
- [x] カーソル位置の管理
  - [x] 現在のバイト位置を保持
  - [x] Hexペイン/ASCIIペインの切り替え
- [x] カーソルの視覚的表示
  - [x] 選択中のバイトをハイライト
  - [x] カーソル位置のインジケーター
- [x] 基本的なキーボードナビゲーション
  - [x] 矢印キー（上下左右）
- [x] 拡張キーボードナビゲーション
  - [x] Page Up / Page Down
  - [x] Home / End
  - [x] Ctrl+Home / Ctrl+End（ファイルの先頭/末尾）
  - [x] Shift+上記キーで範囲選択
- [x] カーソル移動時の自動スクロール
  - [x] キーボード移動時のスクロール追従
  - [x] マウスクリック・ドラッグ時のスクロール追従
- [x] マウス操作
  - [x] クリックでバイト選択
  - [x] ドラッグで範囲選択

#### 2. 選択範囲 ✅ 完了
- [x] 選択範囲の管理（開始位置、終了位置）
- [x] 選択範囲のハイライト表示
  - [x] Hexペインでの表示
  - [x] ASCIIペインでの表示
  - [x] 両ペインの同期
- [x] Shift+矢印キーで範囲選択
- [x] Ctrl+A で全選択

#### 3. 編集機能（拡張）
- [x] バイト編集モード
  - [x] Hexペインでの編集（0-9, A-F入力）
  - [x] ASCIIペインでの編集
  - [x] 入力値の検証
- [x] 挿入/上書きモードの切り替え（Insert / Ctrl+Shift+I）
- [x] 編集バッファの管理（Document構造体で実装済み）
  - [x] 変更箇所の追跡（Overlay HashMap）
  - [ ] 変更済みバイトの視覚的表示
- [x] Undo/Redo
  - [x] 編集履歴のスタック管理
  - [x] Ctrl+Z / Ctrl+Y
  - [x] 履歴の制限（メモリ管理、1000操作）

#### 4. ファイル保存（拡張）
- [x] 現在のファイルに上書き保存
- [x] 保存確認ダイアログ（Ctrl+S時にネイティブダイアログで確認）
- [x] 別名で保存（Ctrl+Shift+S）
  - [x] ファイルダイアログの実装
- [x] 終了時の未保存変更警告
- [ ] 自動バックアップ（オプション）

#### 5. コピー&ペースト ✅ 部分完了
- [x] コピー機能（Cmd+C）
  - [x] ペイン対応スマートコピー（Hexペイン→hex文字列、ASCIIペイン→テキスト）
  - [x] Copy as ASCII Text（非印字文字は `.` に置換）
  - [x] Copy as Hex String（スペース区切り、例: `4A 6F 68 6E`）
  - [x] Copy as C Array（例: `{ 0x4A, 0x6F, 0x68, 0x6E }`）
  - [x] コンテキストメニュー・Editメニューから各形式を選択可能
- [x] ペースト機能（Cmd+V）
  - [x] クリップボードからhex文字列を解析して貼り付け
  - [x] 挿入モード/上書きモード対応
- [ ] カット機能（Cmd+X）

#### 6. 仮想スクロール（パフォーマンス最適化）
**目的**: 大きなファイルでもスムーズに表示・操作できるようにする

**現状の問題**:
- 現在、最大1000行までレンダリングを制限
- 大きなファイル（例：187KB = 11,738行）では最初の1000行しか表示できない
- 全行をレンダリングしようとするとウィンドウが表示されない

##### Phase 1: スクロール位置の追跡 ✅ 完了
- [x] `scroll_offset: Pixels`フィールドを追加
- [x] 行の高さ定数を定義（`ROW_HEIGHT = 20.0px`、Phase 2で使用予定）
- [x] スクロールイベントのハンドリング（render()内で実装）
- [x] ScrollHandleからスクロール位置を取得（offset()メソッド使用）

##### Phase 2: 表示領域の計算 ✅ 完了
- [x] ビューポートサイズの取得方法を調査（ScrollHandle::bounds()使用）
- [x] 表示範囲計算メソッドの実装（calculate_visible_range()）
  - [x] first_visible_row の計算（scroll_offset / row_height）
  - [x] visible_row_count の計算（viewport_height / row_height）
  - [x] バッファ行の追加（前後5行、ちらつき防止）

##### Phase 3: レンダリングの最適化 ✅ 完了
- [x] コンテンツ全体の高さを設定（top + bottom spacer）
- [x] 表示領域内の行のみレンダリング（render_start..render_end）
- [x] 各行の位置を絶対座標で指定（スペーサーによる自動配置）
- [x] スクロール用のスペーサー要素を追加
  - [x] 上部スペーサー（render_start * row_height）
  - [x] 下部スペーサー（(row_count - render_end) * row_height）

##### Phase 4: スクロールイベントの処理 ✅ 完了
- [x] スクロールイベントで`scroll_offset`を更新（render()内で自動処理）
- [x] 再レンダリングのトリガー（gpuiが自動処理）
- [x] スクロールオフセットの符号修正（gpuiでは負の値で下方向）

##### Phase 5: カーソル追従機能 ✅ 完了
- [x] カーソル移動時に表示範囲をチェック（ensure_cursor_visible_by_row()）
- [x] カーソルが画面外の場合、自動スクロール（scroll_to_item()使用）
- [x] スムーズなスクロール体験の実装
- [x] すべてのカーソル移動メソッドに追従処理を追加

##### Phase 6: 大容量ファイル対応 ✅ 完了
- [x] f32精度問題の解決（400MB以上のファイルで発生）
  - [x] 内部計算にf64を使用
  - [x] 仮想コンテンツ高さを1000万ピクセルに制限
  - [x] 比率ベースのスペーサー計算
- [x] 末尾でのレンダリング修正
  - [x] first_visible_rowのクランプ処理

---

### 🟡 中優先度（利便性向上）

#### 7. 検索機能 ✅ 完了
- [x] 検索ダイアログ（Ctrl+F）
- [x] バイナリ検索
  - [x] Hex値での検索（スペース区切り、例: "48 65 6C 6C 6F"）
  - [ ] ワイルドカード対応（"??" で任意のバイト）
- [x] テキスト検索
  - [x] ASCII文字列での検索
  - [ ] 大文字小文字の区別オプション
- [x] 検索結果のナビゲーション
  - [x] 次を検索（F3 / Enter）
  - [x] 前を検索（Shift+F3）
  - [x] すべての結果をハイライト（黄色）
  - [x] 現在の結果をハイライト（オレンジ）
- [x] バックグラウンド検索
  - [x] 別スレッドで検索実行（UIフリーズ防止）
  - [x] 検索キャンセル機能（Esc）
  - [x] 検索進行状況表示（ステータスバーに current/total 表示）
  - [x] 定期的なUI更新（500msごと）
  - [x] メモリマップファイルの再オープン（UIスレッドでのコピー回避）
  - [x] 検索結果の上限（10,000件で打ち切り、truncated表示）
- [x] 検索結果への自動スクロール
- [x] スクロールバーに検索結果マーカー表示
- [x] 検索モード切り替え（Tab: ASCII ⇔ Hex）
- [ ] 置換機能（Ctrl+H）

#### 8. ファイルダイアログ ✅ 部分完了
- [x] 「ファイルを開く」メニュー（Ctrl+O / Cmd+O）
  - [x] ネイティブファイルダイアログの表示
  - [ ] 最近使用したファイルのリスト
- [x] 「名前を付けて保存」（Ctrl+Shift+S）
- [x] ドラッグ&ドロップでファイルを開く

#### 9. 情報表示 ✅ 完了
- [x] ステータスバー
  - [x] 現在のバイト位置表示（Offset）
  - [x] 選択範囲のサイズ表示
  - [x] ファイルサイズ表示（B/KB/MB/GB）
  - [x] カーソル位置の値表示（Hex/Dec/Bin）
- [x] データインスペクター（Ctrl+I で表示/非表示）
  - [x] 選択バイトの解釈表示
  - [x] Int8/16/32/64（符号あり/なし）
  - [x] Float32/64
  - [x] エンディアン切り替え（Ctrl+E: Little/Big）
  - [x] Hex表示（8/16/32/64ビット）
- [ ] オフセット計算ツール
  - [ ] 基数変換（Hex ⇔ Dec ⇔ Bin）
  - [ ] オフセット演算

---

### 🟢 低優先度（追加機能）

#### 10. その他のパフォーマンス最適化
- [x] メモリマップドファイル（Document構造体で実装済み）
  - [x] ファイル全体をメモリに読み込まない
  - [x] オンデマンドでデータを読み込む（10MB以上）
- [x] 検索機能のパフォーマンス最適化
  - [x] 効率的なデータアクセス
    - [x] SearchDataSource列挙型（FilePath/InMemory）
    - [x] メモリマップファイルはバックグラウンドで再オープン
    - [x] UIスレッドでの大量データコピー回避
  - [x] 検索結果マッチング判定の最適化
    - [x] HashSetによるO(1)ルックアップ
    - [x] HashSet構築をバックグラウンドスレッドで実行
    - [x] レンダリング時の効率化（O(n*m) → O(1)）
  - [x] キー入力時のUIブロッキング解消
    - [x] perform_search()内のstd::thread::sleep()を削除
    - [x] 検索キーワード入力がスムーズに
  - [x] 検索結果数の制限（最大10,000件）
- [ ] 差分レンダリング
  - [ ] 変更部分のみを再描画
  - [ ] スクロール時のパフォーマンス向上

#### 11. 高度な機能
- [x] タブ機能
  - [x] 複数ファイルの同時表示
  - [x] タブの切り替え（クリックまたはCtrl+Tab/Ctrl+Shift+Tab）
  - [x] 新規タブ（Ctrl+T）
  - [x] タブを閉じる（Ctrl+W）
  - [x] タブのドラッグ&ドロップ（並び替え）
- [x] 比較モード
  - [x] 2つのファイルを並べて比較
  - [x] 差分のハイライト
  - [x] Ctrl+K: 比較モードの開始/終了
  - [x] タブ選択ダイアログ（数字キーまたはクリックで選択）
- [x] ブックマーク
  - [x] 重要な位置にブックマークを設定（Ctrl+B）
  - [x] ブックマーク間のジャンプ（F2 / Shift+F2）
  - [x] ブックマークコメント機能
    - [x] ブックマーク追加時にコメント入力バーを表示
    - [x] 既存ブックマークのコメント編集（Ctrl+Shift+B）
    - [x] ジャンプ時にコメントをステータスメッセージに表示
  - [x] スクロールバーにブックマークマーカー表示（コメントなし: シアン、コメントあり: グリーン）
  - [x] アドレス列にブックマークインジケーター表示（コメントなし: シアン、コメントあり: グリーン）
  - [x] Hex/ASCIIカラムにブックマークバイトのアンダーライン表示（コメントなし: シアン、コメントあり: グリーン）
- [ ] エクスポート
  - [x] C配列形式でクリップボードにコピー（Copy as C Array）
  - [ ] C配列形式でファイルにエクスポート
  - [ ] Base64エンコード
  - [ ] Hex dump形式
- [x] Bitmap表示
  - [x] バイナリデータをビットマップとして視覚化（Ctrl+M）
  - [x] 幅の調整（+/-キー: 64/128/256/512/1024）
  - [x] カラーマッピング（Cキー: Grayscale/Heatmap/Category）
  - [x] カーソル位置追従とハイライト

#### 12. カスタマイズ
- [ ] 設定画面
  - [ ] フォントサイズ変更
  - [x] カラーテーマ変更（Dark/Light/Monokai、ステータスバーのドロップダウンで切り替え、config.tomlで設定可能）
  - [ ] 1行あたりのバイト数変更（8/16/32）
- [ ] キーボードショートカットのカスタマイズ
- [ ] レイアウトの保存/復元

#### 13. その他
- [ ] コマンドパレット（Ctrl+Shift+P）
- [ ] ヘルプドキュメント
- [ ] チュートリアル
- [ ] 使用統計（オプション）

---

## 🔄 進行中: Document構造体リファクタリング

### 目的
データ管理とUI表示の責務を分離し、巨大ファイル対応とUndo/Redo機能の基盤を構築する。

### アーキテクチャ
- **DataBackend**: 小ファイル用のInMemory、大ファイル用のMemoryMapped
- **Overlay**: 編集バッファ（HashMap<offset, u8>）で変更箇所のみ保持
- **Edit History**: Undo/Redoスタック管理

### タスクリスト

#### Phase 1: Document基本実装 ✅ 完了
- [x] memmap2クレートをCargo.tomlに追加
- [x] src/document.rsファイルを作成
- [x] DataBackend enum実装（InMemory/MemoryMapped）
- [x] Edit構造体実装
- [x] Document構造体の基本定義
- [x] コンストラクタ実装（new, with_data, from_file）
- [x] ファイルI/O実装（load, save, save_as）
- [x] データアクセスメソッド実装（get_byte, get_slice, len）

#### Phase 2: 編集機能 ✅ 完了
- [x] Overlayベースのset_byte実装
- [x] 変更追跡機能（has_unsaved_changes, is_modified）
- [x] Undo/Redo実装（undo, redo, can_undo, can_redo）

#### Phase 3: HexEditorのリファクタリング ✅ 完了
- [x] HexEditor構造体をDocument使用に変更
- [x] データアクセスをdocument経由に移行
- [x] 編集操作をdocument.set_byte()に移行
- [x] ファイルI/Oをdocument経由に移行
- [x] 変更追跡ロジックをDocumentに委譲
- [x] Undo/Redoキーボードショートカット追加（Ctrl+Z / Ctrl+Y）

#### Phase 4: テストと検証（進行中）
- [x] 小ファイル（< 10MB）での動作確認
- [ ] 大ファイル（> 10MB）でのmmap動作確認
- [x] Undo/Redo機能実装完了
- [x] 保存機能動作確認
- [ ] パフォーマンス最適化（大量行のレンダリング問題）

---

## 📝 実装ノート

### 技術的な検討事項

1. **状態管理**
   - カーソル位置、選択範囲、編集バッファなどの状態をどう管理するか
   - gpuiの状態管理パターンに従う

2. **イベントハンドリング**
   - キーボード入力のハンドリング
   - マウスイベントのハンドリング
   - フォーカス管理

3. **描画最適化**
   - 大きなファイルでのパフォーマンス
   - スクロール時のちらつき防止

4. **ファイルI/O**
   - 非同期読み込み（UIブロックを防ぐ）
   - 部分的な読み込み（大容量ファイル対応）

### 次のステップ

推奨実装順序：
1. カーソル機能（位置管理 + 表示）
2. キーボードナビゲーション
3. バイト編集機能
4. 保存機能
5. 選択とコピー&ペースト
6. 検索機能

---

## 使い方（現在）

```bash
# サンプルデータを表示
cargo run

# ファイルを開く
cargo run -- <filepath>
cargo run -- Cargo.toml
```

## ビルドとテスト

```bash
# ビルド
cargo build
cargo build --release

# 実行
cargo run
cargo run -- path/to/file

# テスト
cargo test

# コード品質
cargo clippy
cargo fmt
```
