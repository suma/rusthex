...
//!
//! gpui 0.2.2's Windows backend stores menu data via `cx.set_menus()` but does
//! not create Win32 HMENU native menu bars.  This module works around that by:
//! 1. Building Win32 HMENU menus from gpui `Menu` definitions
//! 2. Subclassing the window to intercept `WM_COMMAND` messages from the menu
//! 3. Queuing gpui `Action`s for deferred dispatch in the next `render()` frame
...
/// Build and attach a native Win32 menu bar to the given gpui `Window`.
///
...
pub fn install_native_menu(window: &Window, menus: &[Menu]) {
    ...
    unsafe {
        ...
        for menu in menus {
            ...
            let wide = to_wide(&menu.name);
            let _ = AppendMenuW(
                popup_to_owner(menu_bar),
                MF_STRING | MF_POPUP,
                hmenu_as_usize(popup),
                PCWSTR::from_raw(wide.as_ptr()),
            );
        }

        let _ = SetMenu(hwnd, Some(menu_bar));
        SetWindowSubclass(hwnd, Some(menu_subclass_proc), SUBCLASS_ID, 0);
    }
}
...
/// Identity helper — menu bar and popup are both HMENU, no conversion needed.
/// Exists only for readability at call-sites.
fn popup_to_owner(
    h: windows::Win32::UI::WindowsAndMessaging::HMENU,
    ...
    for item in items {
        match item {
            ...
            MenuItem::Submenu(submenu) => {
                if let Ok(sub_popup) = CreatePopupMenu() {
                    build_popup_items(sub_popup, &submenu.items, next_id);
