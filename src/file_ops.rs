...
impl HexEditor {
    ...
    /// Run forensic analysis on the current selection using the binary-analysis crate.
    pub fn analyze_binary(&mut self, cx: &mut Context<Self>) {
        let (sel_start, sel_end) = match self.selection_range() {
            Some(range) => range,
            None => {
                self.log(
                    crate::log_panel::LogLevel::Info,
                    "No selection — select a range to analyze",
                );
                cx.notify();
                return;
            }
        ...
        let report = binary_analysis::analyze(&bytes);
        ...
        self.log(
            ...
            format!(
                ...
                report.size,
                report.entropy.unique_bytes,
                report.entropy.data_class,
            ),
        );
        ...
        if report.signatures.is_empty() {
            self.log(
                crate::log_panel::LogLevel::Info,
                "Signature: no known file signatures detected",
            );
        } else {
            for sig in &report.signatures {
                let conf = match sig.confidence {
                    binary_analysis::Confidence::Definite => "definite",
                    binary_analysis::Confidence::Likely => "likely",
                };
                self.log(
                    crate::log_panel::LogLevel::Info,
                    format!(
                        "Signature: {} ({}) at +{:#x} [{}]",
                        sig.name, sig.mime, sig.offset, conf,
                    ),
                );
            }
        }
        ...
        self.log(
            crate::log_panel::LogLevel::Info,
            format!(
                "Distribution: {:.1}% printable, {:.1}% null, {:.1}% high-byte",
                report.distribution.printable_ratio * 100.0,
                report.distribution.null_ratio * 100.0,
                report.distribution.high_byte_ratio * 100.0,
            ),
        );
