...
use gpui::{
    ...
    deferred, div, img, prelude::*, px, rgb, uniform_list,
};
...
/// Embedded third-party license text, generated by `cargo about generate`.
const LICENSES_TEXT: &str = include_str!("../assets/licenses.txt");

/// Lazily split the embedded license text into lines for the About dialog list.
fn license_lines() -> &'static [&'static str] {
    use std::sync::OnceLock;
    static LINES: OnceLock<Vec<&'static str>> = OnceLock::new();
    LINES.get_or_init(|| LICENSES_TEXT.lines().collect())
}
...
impl HexEditor {
    ...
    pub(crate) fn render_about_dialog(&self, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        let t = &self.theme;
        let t_bg_primary = t.bg_primary;
        let t_border_primary = t.border_primary;
        let t_text_secondary = t.text_secondary;
        let licenses_scroll = self.about_licenses_scroll.clone();
        let mono_font = self.settings.display.font_name.clone();
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                ...
                                .track_scroll(licenses_scroll.clone())
                                .h_full()
                                .w_full(),
                            )
                            .child(
                                div()
                                    ...
                                    .w(px(12.0))
                                    .child(
                                        Scrollbar::vertical(&licenses_scroll)
                                            .scrollbar_show(ScrollbarShow::Scrolling),
                                    ),
                            ),
                    )
